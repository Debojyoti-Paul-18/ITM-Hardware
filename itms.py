...
def update_traffic_lights(root, road_names, snapped_points, traffic_lights, api_key, life_cycle_seconds):
    ...
    # Add ESP8266 communication
    esp_master_url = "http://192.168.31.114" # Replace with master ESP's IP
    
    def send_to_esp(phase, traffic_data):
        try:
            requests.post(f"{esp_master_url}/update", 
                json={
                    "phase": phase,
                    "traffic": traffic_data,
                    "junction_type": "T" if len(road_names) <= 3 else "CROSS"
                })
        except Exception as e:
            print(f"Failed to communicate with ESP: {e}")
    
    # Send updates to ESP during traffic light changes
